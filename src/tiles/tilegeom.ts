...
export const BASE_KIND: Record<'A1' | 'A2' | 'A3' | 'A4', number> = {
  A1: 0,
  A2: 16,
  A3: 48,
  A4: 80,
 ...
 */
export function autotileSample(kind: number): [number, number][] {
  const tx = kind % 8;
  ...
  return table.map(([qsx, qsy]) => [(bx * 2 + qsx) * Q, (by * 2 + qsy) * Q] as [number, number]);
}
 ...
 */
export function flatSample(localIndex: number): { sx: number; sy: number } {
