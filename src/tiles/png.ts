/**
 * Minimal, dependency-free PNG codec (Node built-in `zlib` only) — the `src/`
 * (server-build) counterpart of the standalone codec in
 * `.claude/skills/tileset-catalog/scripts/png.mjs`.
 *
 * Just enough to read the PNG sheets RPG Maker MZ ships (8-bit palette, RGB,
 * grayscale, or RGBA) into a flat RGBA buffer — so the transparency classifier
 * can inspect a tile's alpha channel without pulling in `sharp`/`canvas`/`pngjs`.
 * Not a general PNG library: 8-bit channels only, no interlacing (RPG Maker
 * never interlaces). `encodePng` is included for tests/round-trips; the server
 * only needs `decodePng`.
 */
import zlib from 'node:zlib';

const SIG = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);

/** A decoded image: `data` is `width * height * 4` RGBA bytes. */
